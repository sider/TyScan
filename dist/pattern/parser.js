...
const parser_1 = require("./type/parser");
...
function parse(patterns) {
    ...
    return new pattern_1.Pattern(exprs);
}
exports.parse = parse;
const parser = P.createLanguage({
    ...
    TypeAnnotation: L => L.COLON.then(parser_1.typeParser).trim(P.optWhitespace),
    ...
    Atom: L => P.alt(L.NOT.then(L.Atom).map(f => new node.Not(f)), L.Expression.wrap(L.LPAREN, L.RPAREN), L.Wildcard, L.NAME.map(t => new node.Identifier(t))).trim(P.optWhitespace),
    Wildcard: L => L.USCORE
